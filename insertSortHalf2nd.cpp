...
void insertSortHalf(T arr[], size_t len, Compare compare)
{
    ...
    for(size_t i = 1; i<len;i++)
    {
        ...
        //compare(arr[left],arr[i]) && compare(arr[i],arr[right]) need insert
        //                 <                         <
...
{
    int arr[] = {3,1,4,1,5,9,2,6,1,1,1};
    insertSortHalf(arr,11,[](const int& left, const int& right){return left<=right;});
    for(int i = 0 ;i<11;i++)
        std::cout<<arr[i]<<' ';
}